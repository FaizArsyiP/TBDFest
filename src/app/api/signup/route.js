...
export async function POST(req) {
    ...
    if (!username || !email || !no_telepon || !password) {
        return NextResponse.json({ error: 'Isi semua kolom yang tersedia' }, { status: 400 });
    }
    ...
    if (existingUser) {
        ...
        return NextResponse.json({ error: 'Email dan Username sudah terdaftar' }, { status: 409 });
    ...
    if(no_telepon.length < 10 || no_telepon.length > 15) {
        return NextResponse.json({ error: 'Nomor telepon harus antara 10 hingga 15 karakter' }, { status: 400 });
    }
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(username)) {
        return NextResponse.json({ error: 'Username hanya boleh mengandung huruf, angka, titik, garis bawah, dan tanda hubung' }, { status: 400 });
    }
    
    if (!/^[0-9]+$/.test(no_telepon)) {
        return NextResponse.json({ error: 'Nomor telepon hanya boleh mengandung angka' }, { status: 400 });
    }
    ...
    if (lastUserError) {
        return NextResponse.json({ error: 'Database error saat mengambil id_pengguna terakhir' }, { status: 500 });
    }
    ...
    return NextResponse.json({ message: 'Pendaftaran berhasil', user: insertedUser }, { status: 201 });
}
